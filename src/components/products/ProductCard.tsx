"use client"; // Required for using hooks like useQueryClient

import React from 'react';
import Image from 'next/image'; // Use Next.js Image for optimization
import Link from 'next/link'; // Link to product detail page
import { useQueryClient } from '@tanstack/react-query'; // For caching and data fetching
import { Product } from '@/lib/types'; // Import the Product type
import { getProductById } from '@/lib/api'; // Import the API function to fetch product details
  ...
  */
  const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
    ...
    const prefetchProduct = () => {
      console.log(`Prefetching product ID: ${product.id}`);
      queryClient.prefetchQuery({
        queryKey: ['product', product.id], // Use the same query key as the detail page
        queryFn: () => getProductById(product.id),
        // Set a stale time to avoid refetching immediately
        staleTime: 10 * 1000, // 10 seconds
      });
    };

    return (
      // Add onMouseEnter to prefetch product details when the card is hovered
      <Link href={`/products/${product.id}`} className="group block" onMouseEnter={prefetchProduct}> {/* Link wrapping the card */}
