...
export default function ProductsPage() {
  ...
  return (
    <div>
      ...
      <div className="mb-8">
        <h2 className="text-xl font-semibold mb-4 text-text-light dark:text-text-dark">Filter by Category</h2>
        ...
        {isErrorCategories && (
          <p className="text-red-600 dark:text-red-400">Could not load categories.</p>
        )}
        {/* Display category filters/buttons */}
        {categories && (
          <div className="flex flex-wrap gap-2">
          {/* "All Categories" Button */}
            <button onClick={() => handleSelectCategory(null)} className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors duration-200 ${selectedCategory === null
              ? 'bg-primary text-white shadow-md'
              : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600'}`}>All
            </button>
            {/* Buttons for each category */}
            {categories.map((category) => (
              <button key={category} onClick={() => handleSelectCategory(category)} className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors duration-200 capitalize ${selectedCategory === category
                ? 'bg-primary text-white shadow-md'
                : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600'}`}>{category}
              </button>
              ))}
          </div>
        )}
      ...
      <div>
        ...
        {isLoading && (
          <div className="text-center py-10">
            <p className="text-lg font-semibold text-gray-600 dark:text-gray-400">{isLoadingCategories ? `Loading categories...` : `Loading products...`}</p>
             {/* Grid of skeleton loaders */}
             <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mt-6">
               {Array.from({ length: ITEMS_PER_PAGE ? 4 : 8 }).map((_, index) => ( // Show fewer skeletons when filtering
        ...
        {!isLoadingProducts && isErrorProducts && (
          <div className="text-center py-10 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/50 rounded-lg p-6">
            ...
            <p className="text-red-600 dark:text-red-400">
              {productsError?.message || 'An unexpected error occurred. Please try again later.'}
            </p>
          </div>
        )}

      {/* === Empty State (Products) === */}
      {!isLoadingProducts && !isErrorProducts && (!allProducts || allProducts.length === 0) && (
          <div className="text-center py-10">
            <p className="text-lg font-semibold text-gray-600 dark:text-gray-400">No products found{selectedCategory ? ` in "${selectedCategory}"` : ''}.</p>
      ...
      {!isLoadingProducts && !isErrorProducts && paginatedProducts.length > 0 && (
