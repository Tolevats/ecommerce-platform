...
const StarRating: React.FC<{ rate: number; count: number }> = ({
  rate,
  count,
}) => {
...
export default function ProductDetailPage() {
  ...
  const { mutate: addItemMutate, isPending: isAddingToCart } = useMutation({
    ...
    mutationFn: async (productToAdd: Product) => {
      try {
        console.log(
          "AddItem MutationFn: Adding product to Zustand store",
          productToAdd.id
        );
    ...
    onError: (error, variables) => {
      ...
      toast.error(
        `Failed to add ${variables.title.substring(0, 20)}... to cart.`
      );
    },
    onSettled: () => {
  ...
  const handleAddToCart = () => {
    if (product && isMounted) {
      // Ensure product and component are mounted
      ...
      <div className="container mx-auto px-4 py-10 animate-pulse">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
          ...
          <div>
            ...
            {/* Title */}
            <div className="h-6 bg-gray-300 dark:bg-gray-700 rounded w-1/5 mb-6"></div>{" "}
            {/* Rating */}
            <div className="h-5 bg-gray-300 dark:bg-gray-700 rounded w-full mb-3"></div>{" "}
            {/* Description line 1 */}
            <div className="h-5 bg-gray-300 dark:bg-gray-700 rounded w-full mb-3"></div>{" "}
            {/* Description line 2 */}
            <div className="h-5 bg-gray-300 dark:bg-gray-700 rounded w-3/4 mb-6"></div>{" "}
            {/* Description line 3 */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12 mb-16">
          ...
          <div className="flex flex-col justify-center">
            ...
            <button
              ...
              className="w-full md:w-auto px-8 py-3 bg-primary text-white font-semibold rounded-lg shadow-md hover:bg-primary-dark focus:outline-none focus:ring-2 focus:ring-primary-light focus:ring-offset-2 dark:focus:ring-offset-background-dark transition duration-200 ease-in-out disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center"
